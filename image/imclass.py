...
class Mika():
    ...
    def initialise(self,):
        ...
        if self.dirs is not None:
            ...
            self.volume_fpath = self.dirs['mika'] + 'volume0.hdf5'
...
#                 print('loaded anatomical reference')
        ...
        self.load_volume_anat()
        
       
    def load_volume_anat(self,):
        self.anat = h5py.File(self.volume_fpath, 'r')['volume_mean'][()]
#         print(self.anat)
        bkg = self.background.mean()
        self.anat = np.clip(self.anat-bkg, 0, 100)
        self.anat /= self.anat.max()
        
        self.anat_rgb = self.convert_2rgb(self.anat)
        self.anat_rgb[:,:,:,0] = 0
        self.anat_rgb[:,:,:,2] = 0
        
    def convert_2rgb(self, im):
        if len(im.shape) ==2:
            return np.repeat(im[:,:,None], 3, axis = 2)
        if len(im.shape) ==3:
            return np.repeat(im[:,:,:, None], 3, axis = 3)
...
#         self.baseline[self.non_valid] = self.background.min()
    ...
    def fill_anat_vol(self,):
        cmap = pl.get_cmap('tab20',100)
        rand = list(np.random.uniform(low = 0, high = 1, size = self.n))
        data_tn = cmap(rand)[:,:3].T
        data_tn_z = np.ones(self.n)

        vol = self.fill_vol(data_tn).transpose([1,2,3,0])
        volz = self.fill_one_vol(data_tn_z)
        return vol, volz
    ...
    def make_segment_mask(self,):
        ...
        pl.imshow(vol.max(1).transpose([2,1,0]))
                  
        figpath = self.fig_path + 'segement_map.png'
        pl.savefig(figpath)
        rgb = np.concatenate([vol.transpose([1,0,3,2]), self.anat[:,None]], axis = 1)
        
        path_anat = self.dirs['anat'] + 'anat_mkref'
        ext = '.tif'
        tf.imsave(path_anat + ext, data = self.anat[:,None].astype('float32'), imagej = True)
    ...
    def fill_one_vol(self, data):
        ...
        vol[self.cell_z[self.valid_cx].flatten(), \
            self.cell_x[self.valid_cx].flatten(), \
            self.cell_y[self.valid_cx].flatten()] = data[self.valid_cx]
        return vol
    ...
    def clean_up(self, data_nt, indices, post = 5, maxT = None):
        tmp = np.copy(data_nt)
        for i in indices:
            tmp[:, i:i+post] = data_nt[:,i-post*3:i-post*2].mean(axis = 1)[:,None]
        if maxT is not None:
            tmp = tmp[:,:maxT]
        return tmp
    
    def clean_up_range(self, data_nt, indices, post = 5, pre = 0, maxT = None, avg_over = 10, space = 4):
        tmp = np.copy(data_nt)
        for i in indices:
            tmp[:, i[0]-pre:i[1]+post] = data_nt[:,i[0]-space-avg_over:i[0]-space].mean(axis = 1)[:,None]
        if maxT is not None:
            tmp = tmp[:,:maxT]
        return tmp
    ...
    def applyRasterMap(self, data_nt, k = 30, save = True, name = ''):
        ...
        if save:
            try:
                self.raster_path = self.dirs['factors'] + 'raster.npy'
                raster = {}
                raster['raster'] = self.raster
                raster['order'] = self.order
                np.save(self.raster_path, raster)
            except:
                pass
        return raster, order
...
def overlay(vol, anat, alphas = [.5, .5], gamma = .5): 
    ims = []
    z = vol.shape[0]
    vol /= vol.max()
    for ind in range(z):
        ...
        im1[:,:,2] = 0
        im2[:,:,1:2] = 0
        # ims.append(cv2.addWeighted(im1, alphas[0], im2, alphas[1], gamma))
    return np.array(ims)
