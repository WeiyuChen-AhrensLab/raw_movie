...
class ZDS(object):
    def __init__(self, experiment_path, affines=None, single_plane=False):
        ...
        param_file = self.path + 'ch0*'
        param_file = glob(param_file)[0]
        self.metadata = get_metadata(param_file)
...
def get_data_zds(imag_folder, dt = 100, ds = 2, func = None, slice_ = None):
    dset = ZDS(imag_folder)
    fs_im = dset.metadata['volume_rate']
    print('fs_im: %.2f' % fs_im)
    data = dset.data
    print('original shape:', data.shape)
    if func is not None:
        X = func(data[::dt,:,::ds,::ds], axis = 1).compute()
    elif slice_ is not None:
        X = data[::dt, slice_].compute()
    else:
        X = data[::dt,:,::ds,::ds].compute()
    print('shape:', X.shape)
    print('data computed')
    return X, dset.metadata['data_header']
...
def get_metadata(param_file):
    ...
    if type(exp_dict["dimensions"]) is str:
        ...
        exp_dict["dimensions"] = array(exp_dict["dimensions"].split(';')[0].split("x")).astype("int")
